...
4. **Scheduled Task sob demanda** (ver `CONHECIMENTO-SUPERVISORES.md`): se
   `SupAutomacaoUteis-SubAgent-<modulo>` estiver desabilitada (fila estava vazia, ela se
   autodesabilita), reabilite-a agora (`Enable-ScheduledTask -TaskName
   "SupAutomacaoUteis-SubAgent-<modulo>"`) — sem isso a tarefa fica parada até alguém lembrar de
...
3. Quando ele responder, edite o arquivo: preencha `Resposta:` e mude `Status` para `respondida`.
   Se `SupAutomacaoUteis-SubAgent-<modulo>` estiver desabilitada (Scheduled Task sob demanda, ver
