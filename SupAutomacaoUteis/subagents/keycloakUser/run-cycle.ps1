# Executado pela Scheduled Task "SupAutomacaoUteis-SubAgent-keycloakUser" a cada 5 minutos.
# Roda um único ciclo do subAgent do módulo "keycloakUser" em modo não interativo.
...
$nomeTaskAgendada = "SupAutomacaoUteis-SubAgent-keycloakUser"
...
if (-not (Test-TrabalhoPendente)) {
    ...
    "$ts | [ciclo pulado] sem tarefa pendente/retomavel/respondida - claude nao foi chamado" |
        Add-Content -Path (Join-Path $PSScriptRoot "run-log.txt") -Encoding utf8
    Atualizar-FilaTarefas -NomeModulo "SupAutomacaoUteis/keycloakUser" -IdTarefa $null
    Set-CadenciaAdaptativa -Estado 'ocioso' -LogPath (Join-Path $PSScriptRoot "run-log.txt")
    exit 0
}

$idTarefaAtual = (Get-ChildItem -Path "tarefas/executando" -File -ErrorAction SilentlyContinue | Select-Object -First 1).BaseName
$contaEfetiva = Adquirir-SlotConta -NomeModulo "SupAutomacaoUteis/keycloakUser" -IdTarefa $idTarefaAtual -LogPath (Join-Path $PSScriptRoot "run-log.txt")
if (-not $contaEfetiva) {
    Set-CadenciaAdaptativa -Estado 'ocioso' -LogPath (Join-Path $PSScriptRoot "run-log.txt")
    exit 0
}
...
3. No repositório em repo/: se está começando a tarefa agora, dê pull na branch reviewAgents e crie
   ...
   ambientes keycloak/keycloakProd) e o helper de busca por nome exato
   (cypress/support/shared/keycloakHelpers.js, encontrarPorNomeExato) — não reimplemente essas
...
6. Se travar numa dúvida bloqueante — inclusive qualquer decisão sobre QUAL usuário copiar, QUE
   ...
   tarefa). Nunca responda sua própria dúvida. Nunca decida sozinho um desses pontos sensíveis.

Nunca trabalhe em mais de uma tarefa ativa por vez. Nunca exponha senha/token em
docs/documentacao.md, duvidas.md, log, ou no aviso pro Agent Master.
'@
