...
$nomeTaskAgendada = "SupAutomacaoUteis-SubAgent-cedente"
...
# Enquanto C:\Multiplica\claudeAgents\PAUSA-HML.flag existir, este processo depende do SQL Server
# HML (e potencialmente de login na plataforma pra baixar documento via mc-documento-ms) e por
# isso não tenta nenhum ciclo — evita gastar tokens/tempo retentando contra um ambiente
# confirmadamente fora do ar. Removido pelo Thiago (ou por mim a pedido dele) assim que o ambiente
# for restabelecido.
...
if (-not (Test-TrabalhoPendente)) {
    ...
    "$ts | [ciclo pulado] sem tarefa pendente/retomavel/respondida - claude nao foi chamado" |
        Add-Content -Path (Join-Path $PSScriptRoot "run-log.txt") -Encoding utf8
    Atualizar-FilaTarefas -NomeModulo "SupAutomacaoUteis/cedente" -IdTarefa $null
    Set-CadenciaAdaptativa -Estado 'ocioso' -LogPath (Join-Path $PSScriptRoot "run-log.txt")
    exit 0
}

$idTarefaAtual = (Get-ChildItem -Path "tarefas/executando" -File -ErrorAction SilentlyContinue | Select-Object -First 1).BaseName
$contaEfetiva = Adquirir-SlotConta -NomeModulo "SupAutomacaoUteis/cedente" -IdTarefa $idTarefaAtual -LogPath (Join-Path $PSScriptRoot "run-log.txt")
if (-not $contaEfetiva) {
    Set-CadenciaAdaptativa -Estado 'ocioso' -LogPath (Join-Path $PSScriptRoot "run-log.txt")
    exit 0
}
...
1. A fila de tarefas já foi sincronizada deterministicamente antes deste ciclo (dúvida respondida
   ...
   RETOME-A (um ciclo anterior pode ter esgotado antes de terminar, o que é esperado numa tarefa
   deste tamanho): procure em repo/ uma branch já criada para essa tarefa (nome derivado do
...
4. Rode um autoteste sobre sua própria implementação antes de considerar concluído (npm run lint,
...
6. Se travar numa dúvida bloqueante — inclusive qualquer decisão sobre incluir uma tabela fora do
...
Sync-RepoRaizClaudeAgents -LogPath (Join-Path $PSScriptRoot "run-log.txt") -PermitirCommitEPush -MensagemCommit "cedente: sincroniza estado do ciclo (auto, $(Get-Date -Format 'yyyy-MM-dd HH:mm'))"
