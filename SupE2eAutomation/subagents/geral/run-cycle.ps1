# Executado pela Scheduled Task "SupE2eAutomation-SubAgent-geral" a cada 15 minutos.
# Roda um único ciclo do subAgent do módulo "geral" em modo não interativo.
...
$nomeTaskAgendada = "SupE2eAutomation-SubAgent-geral"
...
if (-not (Test-TrabalhoPendente)) {
    ...
    "$ts | [ciclo pulado] sem tarefa pendente/retomavel/respondida - claude nao foi chamado" |
        Add-Content -Path (Join-Path $PSScriptRoot "run-log.txt") -Encoding utf8
    Atualizar-FilaTarefas -NomeModulo "SupE2eAutomation/geral" -IdTarefa $null
...
$contaEfetiva = Adquirir-SlotConta -NomeModulo "SupE2eAutomation/geral" -IdTarefa $idTarefaAtual -LogPath (Join-Path $PSScriptRoot "run-log.txt")
if (-not $contaEfetiva) {
    Set-CadenciaAdaptativa -Estado 'ocioso' -LogPath (Join-Path $PSScriptRoot "run-log.txt")
    exit 0
}
...
$prompt = @'
Você é o SubAgent do módulo "geral" (automação de UI). Leia AGENTE.md nesta pasta e siga suas
...
3. No repositório em repo/: se está começando a tarefa agora, dê pull na branch reviewAgents e crie
   ...
   seguir o padrão do README.md do repo, atualizando-o se necessário). Se o ciclo for esgotar antes
   de terminar, faça commit do progresso parcial na branch (mesmo incompleto) — não deixe o
   trabalho só em arquivos não commitados, senão o próximo ciclo não consegue retomar.
...
5. Se concluir com sucesso: siga a regra 6 do AGENTE.md (commit + push da branch, aviso em
   ../../agent-master/fila-merge/pendentes/ com a branch e o id da tarefa, atualize
...
Sync-RepoRaizClaudeAgents -LogPath (Join-Path $PSScriptRoot "run-log.txt") -PermitirCommitEPush -MensagemCommit "geral (E2e): sincroniza estado do ciclo (auto, $(Get-Date -Format 'yyyy-MM-dd HH:mm'))"
