...
# Conta do Claude Code dedicada a este subAgent (3º módulo criado -> volta pra contaA).
...
2. Se tarefas/executando/ já tiver uma tarefa, RETOME-A (um ciclo anterior pode ter esgotado antes
   ...
   do zero, não descarte trabalho já feito/descoberto); se não houver nenhuma branch/progresso,
   trate como se estivesse começando agora. Prossiga a partir do passo 5.
...
7. Rode um autoteste sobre sua própria implementação antes de considerar concluído. Confirme que o
...
9. Se travar numa dúvida bloqueante: siga a regra 8 do AGENTE.md (registre em duvidas.md no
