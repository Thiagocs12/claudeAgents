# Histórico — duvidas.md (arquivado)

Entradas movidas para cá a partir de `duvidas.md` (arquivamento por tamanho, ver regra 10 do
`AGENTE.md`). Nenhuma delas correspondia, no momento do arquivamento, a uma tarefa fisicamente
presente em `tarefas/aguardando-resposta/` (confirmado antes de mover — ver protocolo de segurança
sobre esse ponto). Conteúdo preservado verbatim, nada foi descartado.
