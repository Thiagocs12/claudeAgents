# Executado pela Scheduled Task "SupE2eAutomation-AgentMaster" a cada 30 minutos.
...
# houver algo pra processar em fila-merge/. A garantia do PR único reviewAgents->main (item 2 da
...
# o claude. Ver seção 3.4 do CLAUDE.md.
...
aprovação humana por tarefa. Você NUNCA mergeia/dá push direto na main: o único ponto de revisão
manual do Thiago é um PR unico e continuo reviewAgents para main, que voce garante que existe
(cria uma vez se faltar, nunca recria) e que reflete sozinho no GitHub cada commit novo pusheado
na reviewAgents. gh CLI ja autenticado via GH_TOKEN. Execute agora UM unico ciclo da logica
operacional:

1. Leia ../docs/conhecimento-geral.md (raiz do Supervisor) inteiro, depois docs/documentacao.md
   inteiro.
2. Garanta o PR unico: rode gh pr list --base main --head reviewAgents --state open. Se nao
   existir nenhum aberto, crie um com gh pr create (base main, head reviewAgents, titulo curto
   tipo Integracao continua reviewAgents para main, corpo com um resumo do que esta pendente de
   revisao). Se ja existir, nao mexa nele.
3. Legado: se houver algo em fila-merge/aguardando-aprovacao/ (aviso de PR por tarefa do modelo
...
5. Depois de processar os avisos, sincronize C:\multiplica\cypress-e2e sempre para a reviewAgents
...
6. Registre em docs/documentacao.md tudo que foi feito (merges feitos direto na reviewAgents,
   conflitos resolvidos, variáveis de .env novas só o nome, estado do PR único pra main, e estado
   da sincronização da pasta de teste manual). Se o aprendizado valer para qualquer módulo,
