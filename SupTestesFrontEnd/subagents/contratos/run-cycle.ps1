# Executado pela Scheduled Task "SupTestesFrontEnd-SubAgent-contratos" a cada 10 minutos.
# Roda um único ciclo do subAgent do módulo "contratos" (Sup TestesFrontEnd) em modo não interativo.

$ErrorActionPreference = "Continue"
Set-Location -Path $PSScriptRoot

# --- Sincronização automática do repo raiz (claudeAgents) ---
# Ver CONHECIMENTO-SUPERVISORES.md, seção "git add/git commit no repo raiz" — mesmo padrão
# replicado de SupTestesFrontEnd/subagents/mop/run-cycle.ps1, não reinventado aqui.
...
# plataforma (HML) e por isso não tenta nenhum ciclo.
...
# Conta do Claude Code — contaB é a conta padrão (ver CONHECIMENTO-SUPERVISORES.md, "Pool de
# contas"), contaA só entra como fallback de rate-limit.
...
# Rede de segurança determinística contra processo Cypress/node órfão (ver
# CONHECIMENTO-SUPERVISORES.md, "Armadilhas de ambiente compartilhadas" — mesmo padrão de
# subagents/mop/run-cycle.ps1).
...
$prompt = @'
Você é o SubAgent do módulo "contratos" (Sup TestesFrontEnd — QA exploratório, não automação
persistente). Leia AGENTE.md nesta pasta e siga suas regras fixas à risca (inclui o ponteiro pra
especificação de apoio em AgenteEspecificacao/especificacoes/contratos-qcertifica/). Execute agora
UM único ciclo da lógica operacional:
...
3. Tente cumprir o objetivo da tarefa via Cypress (npx cypress run), incrementalmente: escreva um
   ...
   medida que for acontecendo. Chame cy.screenshot('<passo-N-descricao>') manualmente antes/depois
   de cada clique relevante (par "demonstração de clique"). Nunca inicie processo em segundo plano
   e encerre o ciclo esperando ele terminar — rode tudo de forma síncrona dentro do ciclo. Se o
   ciclo for esgotar antes de terminar, garanta que a seção "## Execução" está atualizada com o que
   já foi descoberto/tentado, para o próximo ciclo continuar de onde parou.
4. Ao concluir (objetivo cumprido, cumprido parcialmente, ou travado por um problema real da
   aplicação — isso é RESULTADO, não dúvida): gere o PDF do relatório (script
...
Sync-RepoRaizClaudeAgents -LogPath (Join-Path $PSScriptRoot "run-log.txt") -PermitirCommitEPush -MensagemCommit "contratos (TestesFrontEnd): sincroniza estado do ciclo (auto, $(Get-Date -Format 'yyyy-MM-dd HH:mm'))"
